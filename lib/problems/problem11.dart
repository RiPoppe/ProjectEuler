...
class Grids {
  ...
  static const smallGrid = <List<int>>[
    [1, 4, 3, 4],
    [3, 2, 7, 1],
    [6, 2, 3, 4],
    [1, 9, 3, 4],
...
int problem11({int length = 4, required List<List<int>> grid}) {
  final results = <int>[
    problem11Rows(length: length, grid: grid),
    problem11Columns(length: length, grid: grid),
    problem11Diagonal00to11(length: length, grid: grid),
    problem11Diagonal01to10(length: length, grid: grid)
  ];

  return results.fold(0, (previous, current) => math.max(previous, current));
}

int problem11Rows({int length = 4, required List<List<int>> grid}) {
  int maxSum = 0;
  for (var row = 0; row < grid.length; row++) {
    for (var index = 0; index <= grid.length - length; index++) {
      int sum = 1;
      for (var i = 0; i < length; i++) {
        sum *= grid[row][index + i];
      }
...
int problem11Columns({int length = 4, required List<List<int>> grid}) {
  int maxSum = 0;
  for (var column = 0; column < grid.length; column++) {
    for (var index = 0; index <= grid.length - length; index++) {
      int sum = 1;
      for (var i = 0; i < length; i++) {
        sum *= grid[index + i][column];
      }
      if (sum > maxSum) maxSum = sum;
    }
  }
...
int problem11Diagonal00to11({int length = 4, required List<List<int>> grid}) {
  int maxSum = 0;
  for (var column = 0; column <= grid.length - length; column++) {
    for (var index = 0; index <= grid.length - length; index++) {
      int sum = 1;
      for (var i = 0; i < length; i++) {
        sum *= grid[index + i][column + i];
      }
      if (sum > maxSum) maxSum = sum;
    }
  }
...
int problem11Diagonal01to10({int length = 4, required List<List<int>> grid}) {
  int maxSum = 0;
  for (var row = grid.length - 1; row >= length - 1; row--) {
    for (var column = 0; column <= grid.length - length; column++) {
      int sum = 1;
      for (var i = 0; i < length; i++) {
